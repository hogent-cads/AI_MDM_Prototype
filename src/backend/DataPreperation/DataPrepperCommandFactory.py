...
from src.backend.DataPreperation.Commands.DroppingCommands import (
    DroppingCommand_UniquenessBound,
    DroppingCommand_LowerBound,
    DroppingCommand_UpperBound,
    DroppingCommand_DropNan,
)
    ...
    def parse_cleaning_options_from_json_string(
        ...
        for k, v in cleaning_json_object["binning_option"].items():
            ...
            if binning_command_object_k == BinningEnum.EQUAL_BINS:
                binning_command_object_to_add = BinningCommand_EqualBins(
                    number_of_bins=binning_command_object_v, series=dirty_df[k]
                )
            elif binning_command_object_k == BinningEnum.K_MEANS:
                binning_command_object_to_add = BinningCommand_KMeans(
                    number_of_bins=binning_command_object_v, series=dirty_df[k]
                )
            else:
        ...
        for k, v in cleaning_json_object["dropping_options"].items():
            ...
            for inner_k, inner_v in dropping_command_object.items():
                if inner_k == DroppingEnum.DROP_WITH_UNIQUENESS_BOUND:
                    list_of_dropping_commands.append(
                        DroppingCommand_UniquenessBound(
                            uniqueness_bound=inner_v, dataframe=dirty_df, col=k
                        )
                    )
                elif inner_k == DroppingEnum.DROP_WITH_LOWER_BOUND:
                    list_of_dropping_commands.append(
                        DroppingCommand_LowerBound(
                            lower_bound=inner_v, dataframe=dirty_df, col=k
                        )
                    )
                elif inner_k == DroppingEnum.DROP_WITH_UPPER_BOUND:
                    list_of_dropping_commands.append(
                        DroppingCommand_UpperBound(
                            upper_bound=inner_v, dataframe=dirty_df, col=k
                        )
                    )
                elif inner_k == DroppingEnum.DROP_NAN:
                    list_of_dropping_commands.append(
                        DroppingCommand_DropNan(
                            boolean_in_string=inner_v, dataframe=dirty_df, col=k
                        )
                    )
                else:
            ...
            try:
                df_to_return.loc[len(df_to_return)] = [
                    k,
                    list_of_cleaning_commands,
                    binning_object_dict[k] if k in binning_object_dict else None,
                    list_of_dropping_commands,
                ]
