...
class ValueRuleRepo:
    ...
    def filter_out_column_rule_strings_from_dict_of_value_rules(self, min_support: float) -> List[str]:
        ...
        TODO!!
        """
        self.value_rules_dict: Dict[str, Set[ValueRule]] = \
        ...
        filtered = {rs: max_conf for rs, max_conf in potential_conf_dict.items()
                    if max_conf >= 0.9}
    ...
    def _filter_low_support_rules(self, min_support) -> Dict[str, Set[ValueRule]]:
        ...
        """
        cfg.logger.info("Trying to remove rule_strings" +
        ...
        cfg.logger.info(f"Removed {len(removed_rs)} rule strings because of low support" +
