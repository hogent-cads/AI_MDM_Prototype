...
class CleanerInitPage:
    ...
    def _show_unique_values(self, series):
        ...
        series.apply(
            lambda x: list(
                map(
                    lambda y: value_counts.update({y: value_counts.get(y, 0) + 1})
        ...
        unique_characters_df = pd.DataFrame(
            value_counts.items(), columns=["character", "value_counts"]
        ...
        unique_characters_df = unique_characters_df[
            ~unique_characters_df["character"].str.contains(regex_pattern_to_remove)
        ]
    ...
    def _determine_fill_na_value_based_on_type_and_column(self, type, column):
        try:
            if type == d.DC_CLEANING_FILLNA_VALUE_MEDIAN.value:
                ...
                try:
                    tmp = (
                        st.session_state[Variables.SB_LOADED_DATAFRAME][column][
            ...
            if type == d.DC_CLEANING_FILLNA_VALUE_MODE.value:
                try:
                    ...
                    value_counts = st.session_state[Variables.SB_LOADED_DATAFRAME][
                        column
                    ].value_counts()
    ...
    def _show_cleaning_pipelines_tab(self):
        ...
        with colI_3:
            ...
            if st.button(d.DC_CLEANING_APPLY_PIPELINE_COLUMN.value) & (st.session_state[Variables.DC_PIPELINE] != {}):
                    ...
                    for k, v in item.items():
                        if k == "parameters":
                            if "value" in v:
                                value = v["value"]
                                if value in [
                                    d.DC_CLEANING_FILLNA_VALUE_MEAN.value,
                                    d.DC_CLEANING_FILLNA_VALUE_MEDIAN.value,
                                    d.DC_CLEANING_FILLNA_VALUE_MODE.value,
                                ]:
                                    value = self._determine_fill_na_value_based_on_type_and_column(
                                        value, chosen_column
                                    )
                ...
                st.session_state[Variables.DC_CLEANED_COLUMN] = pd.DataFrame(
                ...
                st.session_state[Variables.DC_CLEANED_COLUMN].index = st.session_state[
                    Variables.DC_CLEANED_COLUMN
                ].index.astype(int)
                st.session_state[Variables.SB_LOADED_DATAFRAME][
                    chosen_column
                ] = st.session_state[Variables.DC_CLEANED_COLUMN][chosen_column]
                st.write("Column successfully cleaned.")

        with colG_2:
            cleaning_methods_list = [
                d.DC_CLEANING_METHOD_LOWERCASE.value,
        ...
        with colI4:
            ...
            if st.button(d.DC_CLEANING_APPLY_PIPELINE_ALL.value) & (st.session_state[Variables.DC_PIPELINE] != {}):
                for e in st.session_state[Variables.SB_LOADED_DATAFRAME].columns:
                    ...
                    st.session_state[
                        Variables.DC_CLEANED_COLUMN
                    ].index = st.session_state[Variables.DC_CLEANED_COLUMN].index.astype(
                        int
                    )
                    st.session_state[Variables.SB_LOADED_DATAFRAME][e] = st.session_state[
                        Variables.DC_CLEANED_COLUMN
                    ][e]
        ...
        with colG_1:
            ...
            st.write(st.session_state[Variables.DC_PIPELINE])

    def _apply_pipeline(self, col):
        st.session_state[Variables.DC_CLEANED_COLUMN] = pd.DataFrame(
            self.handler.clean_dataframe_dataprep(
                dataframe_in_json=st.session_state[Variables.SB_LOADED_DATAFRAME][col]
                .to_frame()
                .to_json(),
                custom_pipeline=st.session_state[Variables.DC_PIPELINE],
            )
        )
        st.session_state[Variables.DC_CLEANED_COLUMN].index = st.session_state[
            Variables.DC_CLEANED_COLUMN
        ].index.astype(int)
        st.session_state[Variables.SB_LOADED_DATAFRAME][col] = st.session_state[
            Variables.DC_CLEANED_COLUMN
        ][col]
    ...
    def _show_structure_detection_tab(self):
        ...
        simple_repr = pd.Series(
            self.handler.structure_detection(
                st.session_state[Variables.SB_LOADED_DATAFRAME][chosen_column].to_json(),
                extra_exceptions,
                compress,
            )
        )

        series_for_aggrid = simple_repr.value_counts(normalize=True).copy(deep=True)
        series_for_aggrid = series_for_aggrid.rename_axis("pattern").reset_index(
            name="percentage"
        )
        ...
        with colB_3:
            if response_patterns["selected_rows"]:
                ...
                selected_pattern = (
                    response_patterns["selected_rows"][0]["pattern"]
                ...
                if selected_pattern is not None:
                    ...
                    df_for_aggrid2 = st.session_state[Variables.SB_LOADED_DATAFRAME][
                        simple_repr.values == selected_pattern
                    ]
                    ...
                    grid = AgGrid(
                        df_for_aggrid2,
                        gridOptions=gridOptions | extra_grid_options_2,
    ...
    def _show_fuzzy_matching_tab(self):
        ...
        with fuzzy_matching_form:
            ...
            with colD_1:
                ...
                if cluster_method == "ngram-fingerprint":
                    ...
                    with colE_1:
                        n_gram = st.slider(
                            "The N for N-Grams", min_value=1, max_value=10, value=2
                        )
                if cluster_method == "levenshtein":
                    colF_1, colF_2 = st.columns([1, 1])
                    with colF_1:
                        radius = st.slider(
                            "The Radius", min_value=1, max_value=10, value=2
                        )
                    with colF_2:
                        block_size = st.slider(
                            "The Block Size", min_value=1, max_value=10, value=6
                        )
        ...
        if fuzzy_matching_btn:
            ...
            for cluster_id, list_of_values in self.handler.fuzzy_match_dataprep(
                ...
                .to_frame()
                .to_json(),
                df_name=st.session_state[Variables.SB_LOADED_DATAFRAME_NAME],
                col=chosen_column,
        ...
        if st.session_state["list_of_fuzzy_cluster_view"] != []:
            ...
            sub_rows_to_use = self._create_pagination(
                "page_number_fuzzy", st.session_state["list_of_fuzzy_cluster_view"], 5
            )
        ...
        for cv in list_of_cluster_view:
            if st.session_state[f"fuzzy_merge_{cv.cluster_id}"]:
                for val_to_replace in cv.selected_values:
                    ...
                    st.session_state[Variables.SB_LOADED_DATAFRAME][column_name] = (
                        st.session_state[Variables.SB_LOADED_DATAFRAME][column_name]
                        .astype(str)
                        .replace(str(val_to_replace), str(cv.new_cell_value))
                    )
    ...
    def _create_cluster_card(
        ...
        with cont:
            ...
            with colA:
                ...
                tmp = AgGrid(
                    cv.distinct_values_in_cluster,
                    gridOptions=gridOptionsFull,
                    enable_enterprise_modules=False,
                    update_mode="SELECTION_CHANGED",
                    height=min(
                        min_height + len(cv.distinct_values_in_cluster) * row_height,
                        max_height,
                    ),
                )
            ...
            with colC:
                ...
                st.session_state[f"new_value_{cv.cluster_id}"] = st.text_input(
                    "New value",
                    value=cv.new_cell_value,
                    key=f"key_new_value_{cv.cluster_id}",
                )
...
class FuzzyClusterView:
    def __init__(self, cluster_id, list_of_values, column_as_series) -> None:
        cfg.logger.debug(
            f"FuzzyClusterView: __init__: cluster_id = {cluster_id},"
        ...
        self.distinct_values_in_cluster = (
            column_as_series[column_as_series.isin(list_of_values)]
            .value_counts()
            .reset_index(name="count")
            .rename(columns={"index": "values"})
        )
        self.merge = False
