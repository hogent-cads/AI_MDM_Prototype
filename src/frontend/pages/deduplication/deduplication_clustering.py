    ...
    def redirect_get_clusters(self):
        ...
        for cluster_id in cluster_ids:
            ...
            if len(records_df) > 1:
                ...
                cluster_confidence = (
                    records_df["z_minScore"].mean() + records_df["z_maxScore"].mean()
                ) / 2
                # new_row is the row that with the highest 'cluster_high' value
                new_row = records_df[
                    records_df["z_minScore"] == records_df["z_minScore"].max()
                ][:1]
                list_of_cluster_view.append(
                    ZinggClusterView(
                        ...
                        cluster_confidence,
                        records_df.drop(
                            ["z_minScore", "z_maxScore", "z_cluster"], axis=1
                        ),
                        new_row,
                        cluster_low,
                        cluster_high,
...
class ZinggClusterPage:
    ...
    def show(self):
        with self.canvas.container():
            ...
            sub_rowstoUse = self._createPaginering(
                "page_number_Dedupe", st.session_state["list_of_cluster_view"], 5
            )
    ...
    def _merge_clusters(self, list_of_cluster_view, pks):
        ...
        for cv in list_of_cluster_view:
            ...
            all_df = pd.merge(
                cv.records_df,
                cv.selected_rows,
                on=list(cv.selected_rows.columns),
                how="left",
                indicator="exists",
            )
    ...
    def _create_cluster_card(self, idx, cv, pks):
        ...
        with cont_card:
            st.session_state[f"merge_{cv.cluster_id}"] = st.checkbox(
                "Apply changes!",
                value=st.session_state[f"merge_{cv.cluster_id}"],
                key=f"key_merge_{cv.cluster_id}",
            )
            ...
            data_clustercard = AgGrid(
                cv.records_df,
                gridOptions=gridOptions,
                enable_enterprise_modules=True,
                update_mode="SELECTION_CHANGED",
                height=min(MIN_HEIGHT + (len(cv.records_df) * ROW_HEIGHT), MAX_HEIGHT),
                key=f"before_{cv.cluster_id}",
            )
            ...
            dedupe_check = st.radio(
                "The selected records will be... ",
                (self.TEXT_DEDUP_FALSE, self.TEXT_DEDUP_TRUE),
                key=f"dedup_{cv.cluster_id}",
                horizontal=True,
            )
            ...
            gb2 = GridOptionsBuilder.from_dataframe(
                cv.new_row.drop(pks, axis=1)
                if dedupe_check == self.TEXT_DEDUP_FALSE
                else cv.new_row
            )
            ...
            grid = AgGrid(
                ...
                height=min(MIN_HEIGHT + ROW_HEIGHT, MAX_HEIGHT),
            )
