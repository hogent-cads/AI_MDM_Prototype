...
class RemoteHandler(IHandler):
    ...
    def create_deduper_object(self, dedupe_type_dict, dedupe_data) -> json:
        ...
        requests.post(f"{self.connection_string}/create_deduper_object", cookies={"session_flask" : st.session_state[VarEnum.gb_SESSION_ID]}, data=json.dumps(data))
    ...
    def dedupe_mark_pair(self, labeled_pair) -> json:
        ...
        requests.post(f"{self.connection_string}/dedupe_mark_pair", cookies={"session_flask" : st.session_state[VarEnum.gb_SESSION_ID]}, data=temp_data)

    def dedupe_get_stats(self) -> json:
        return requests.get(f"{self.connection_string}/dedupe_get_stats", cookies={"session_flask" : st.session_state[VarEnum.gb_SESSION_ID]}).json()

    def dedupe_train(self):
        data = {}
        requests.post(f"{self.connection_string}/dedupe_train", cookies={"session_flask" : st.session_state[VarEnum.gb_SESSION_ID]}, data=json.dumps(data))
    ...
    def prepare_zingg(self, dedupe_type_dict, dedupe_data) -> json:
        ...
        data["dedupe_type_dict"] = dedupe_type_dict
        data["dedupe_data"] = dedupe_data
        return requests.post(f"{self.connection_string}/prepare_zingg", cookies={"session_flask" : f"{st.session_state[VarEnum.gb_SESSION_ID]}-{st.session_state[VarEnum.sb_LOADED_DATAFRAME_HASH]}"}, data=json.dumps(data))
    ...
    def zingg_mark_pairs(self, marked_df) -> json:
        ...
        return requests.post(f"{self.connection_string}/zingg_mark_pairs", cookies={"session_flask" : f"{st.session_state[VarEnum.gb_SESSION_ID]}-{st.session_state[VarEnum.sb_LOADED_DATAFRAME_HASH]}"}, data=json.dumps(data))
