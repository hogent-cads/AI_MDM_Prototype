...
class ZinggLabelPage:
    ...
    def show(self):
        ...
        if VarEnum.DD_CURRENT_LABEL_ROUND not in st.session_state:
            st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND] = pd.DataFrame(self.handler.zingg_unmarked_pairs())
        ...
        with container_for_cards:
            ...
            counter = 0
            for z_cluster_id, z_cluster_df in st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].groupby('z_cluster'):
                counter += 1
        ...
        with stats:
            ...
            with colB_1:
                ...
                st.write(f"Previous Round(s): {st.session_state[VarEnum.DD_LABEL_STATS]['match_files']}/{sum_stats} MATCHES, {st.session_state[VarEnum.DD_LABEL_STATS]['no_match_files']}/{sum_stats} NON-MATCHES, {st.session_state[VarEnum.DD_LABEL_STATS]['unsure_files']}/{sum_stats} UNSURE")
                # st.write("Previous Round(s): " + self.handler.zingg_get_stats())
                st.write("Current labeling round: {}/{} MATCHES, {}/{} NON-MATCHES, {}/{} UNSURE".format(
                    len(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND][st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].z_isMatch == 1]) // 2,
                    len(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND]) // 2,
                    len(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND][st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].z_isMatch == 0]) // 2,
                    len(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND]) // 2,
                    len(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND][st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].z_isMatch == 2]) // 2,
                    len(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND]) // 2
                    ))
            with colB_2:
        ...
        if next:
            _ = self.handler.zingg_mark_pairs(st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].to_json())
        ...
        with cont_card:
            ...
            with colRight:
                if grouped_df['z_prediction'].mean() > 0:
                    ...
                    with colAA:
                        st.write(d.DD_DEDUPLICATION_PREDICTION_SCORE, str(format(grouped_df['z_score'].mean() * 100, '.2f')) + '%')
                    with colBB:
                        st.write(f"{d.DD_DEDUPLICATION_PREDICTION} {d.DD_DEDUPLICATION_LABEL_IS_A_MATCH.value if (grouped_df['z_prediction'].mean() > 0) else d.DD_DEDUPLICATION_LABEL_IS_NOT_A_MATCH.value}")
                else:
                    st.write('')
                    st.write('')
                    st.write('')

                choice = st.selectbox(d.DD_DEDUPLICATION_LABEL_SELECTBOX.value, [d.DD_DEDUPLICATION_LABEL_IS_A_MATCH.value, d.DD_DEDUPLICATION_LABEL_IS_NOT_A_MATCH.value, d.DD_DEDUPLICATION_LABEL_UNSURE.value], index=2, key="selectbox_" + z_cluster_id)
                if choice == d.DD_DEDUPLICATION_LABEL_IS_A_MATCH.value:
                    st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].loc[st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND]['z_cluster'] == z_cluster_id, 'z_isMatch'] = 1
                if choice == d.DD_DEDUPLICATION_LABEL_IS_NOT_A_MATCH.value:
                    st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].loc[st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND]['z_cluster'] == z_cluster_id, 'z_isMatch'] = 0
                if choice == d.DD_DEDUPLICATION_LABEL_UNSURE.value:
                    st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND].loc[st.session_state[VarEnum.DD_CURRENT_LABEL_ROUND]['z_cluster'] == z_cluster_id, 'z_isMatch'] = 2
